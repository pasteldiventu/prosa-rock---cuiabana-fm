...
export function BlogPostCard({ post }: BlogPostCardProps) {
  ...
  return (
    <article className="overflow-hidden rounded-lg border border-zinc-800 bg-dark-surface transition-colors hover:border-zinc-700">
      ...
      <div className="space-y-4 p-6">
        <div className="flex flex-wrap items-center gap-4 text-xs uppercase tracking-wider text-zinc-500">
          ...
          {post.guest && (
            <span className="flex items-center gap-1.5">
              <User className="h-3.5 w-3.5 text-neon-orange" />
              {post.guest}
            </span>
          )}
        </div>
        <h3 className="text-2xl font-display text-white">
          <Link href={`/blog/${post.slug}`} className="hover:text-neon-orange">
            {post.title}
          </Link>
        </h3>
